...
BEGIN_MESSAGE_MAP(CTalksDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_DF, &CTalksDlg::OnDiffie_Hellman)
END_MESSAGE_MAP()
...
BOOL CTalksDlg::OnInitDialog()
{
	...
	m_strServName="localhost";//server name=localhost
	m_nServPort=1000;//server port=1000
	UpdateData(FALSE);
...
void CTalksDlg::OnReceive()
{
	...
	if (nReceived!=SOCKET_ERROR)
	{
		...
		strReceived=pBuf;
		if (strReceived.Find(_T("Diffie_Hellman hello:")) != -1) {
			CString sb = strReceived.Right(8);
		}
...
void CTalksDlg::OnDiffie_Hellman()
{
	// TODO: 在此添加控件通知处理程序代码
	string sa = DS_put(ra);
	CString str = stringToCString(sa);
	//get length of the message
	CString helloMsg = _T("Diffie_Hellman hello:");
	helloMsg += str;
	int nLen = helloMsg.GetLength();
	//send the message
	USES_CONVERSION;
	int nSent = m_sConnectSocket.Send(LPCTSTR(helloMsg), nLen);
